...
public class SOCPlayer implements SOCDevCardConstants, Serializable, Cloneable
{
    ...
    // Ryan's added fields
    private double totalResources = 0.0;
    private double turnCount = 0.0;
    private double[] expectedResources = new double[SOCResourceConstants.WOOD];
    double totalExpectedResources = 0.0;
    ...
    public SOCPlayer(SOCPlayer player)
    {
    	...
    	totalExpectedResources = player.totalExpectedResources;
        turnCount = player.turnCount;
        for(int i=0; i<expectedResources.length; i++)
        	expectedResources[i] = player.expectedResources[i];
    ...
    /**
     * Added by Ryan, copy constructor for SOCPlayer
     * @param socPlayer
     * @param socGame
     */
    public SOCPlayer(SOCPlayer p, SOCGame g) {
    	/*
		name = p.name;
		playerNumber = p.playerNumber;
		game = g;
		numPieces = copy(p.numPieces);
		pieces = (Vector) p.pieces.clone();
		roads = (Vector) p.roads.clone();
		settlements = (Vector) p.settlements.clone();
		cities = (Vector) p.cities.clone();
		lastSettlementCoord = p.lastSettlementCoord;
		lastRoadCoord = p.lastRoadCoord;
		longestRoadLength = p.longestRoadLength;
		lrPaths = (Vector) p.lrPaths.clone();
		resources = new SOCResourceSet(p.resources);
		totalResources = p.totalResources;
		turnCount = p.turnCount;
		expectedResources = copy(p.expectedResources);
		totalExpectedResources = p.totalExpectedResources;
		resourceStats = copy(p.resourceStats);
		devCards = new SOCDevCardSet(p.devCards);
		numKnights = p.numKnights;
		buildingVP = p.buildingVP;
		finalTotalVP = p.finalTotalVP;
		needToDiscard = p.needToDiscard;
		roadNodes = p.roadNodes.clone();
		*/
    	this(p);
    	game = g;
	}

    // Added by Ryan, returns a copy of an int array
    ...
    // Added by Ryan, returns a copy of a double array
    ...
    // Added by Ryan, returns a copy of a Vector array
    private Vector[] copy(Vector[] arr) {
    	if(arr == null)
    		return null;
    	Vector[] ret = new Vector[arr.length];
    	for(int i=0; i<ret.length; i++)
    		ret[i] = (Vector) arr[i].clone();
    	return ret;
    }
    
    // Added by Ryan, returns a copy of a HashTable
    private Hashtable copy(Hashtable h) {
    	if(h == null)
    		return null;
    ...
    public void addRolledResources(SOCResourceSet rolled)
    {
        ...
        for (int rtype = SOCResourceConstants.CLAY; rtype < resourceStats.length; ++rtype) {
            ...
            totalResources += rolled.getAmount(rtype);
        }
        
        // Ryan: this is a hack, but addRolledResources is called exactly once per turn, so I'll update
        // certain administrative methods here.
        double[] thisTurnExpRes = new double[expectedResources.length];
        for(int i=0; i<thisTurnExpRes.length; i++)
        	thisTurnExpRes[i] = 0.0;
        
        for(int i=2; i<=12; i++) {
            SOCResourceSet thisTurnRes = new SOCResourceSet();
        	thisTurnRes.add(game.getResourcesGainedFromRoll(this, i));
        	for (int rtype = SOCResourceConstants.CLAY; rtype <= thisTurnExpRes.length; ++rtype) {
            	thisTurnExpRes[rtype-1] += rolled.getAmount(rtype)*prob(i);
            }
        }
        
        for(int i=0; i<expectedResources.length; i++) {
        	expectedResources[i] = (turnCount*expectedResources[i] + thisTurnExpRes[i]) / (turnCount+1);
        }
    ...
    private double prob(int i) {
    	switch(i) {
    		case 2: return 1.0/36.0;
    		case 3: return 2.0/36.0;
    		case 4: return 3.0/36.0;
    		case 5: return 4.0/36.0;
    		case 6: return 5.0/36.0;
    		case 7: return 6.0/36.0;
    		case 8: return 5.0/36.0;
    		case 9: return 4.0/36.0;
    		case 10: return 3.0/36.0;
    		case 11: return 2.0/36.0;
    ...
    public boolean[] getPotentialSettlements(){
    	return potentialSettlements.clone();
    }
    
    /** Added by Katie.
     * 
     * @return number of potential settlements
     */
    public int numPotentialSettlements(){
    	int sum=0;
    	for(boolean b : potentialSettlements){
    		if (b)
    		sum++;
    	}
    	return sum;
    ...
    public boolean[] getPotentialCities(){
    	return potentialCities.clone();
    }
    
    /** Added by Katie */
    public boolean[] getPotentialRoads(){
    	return potentialRoads.clone();    	
    }
    ...
    public double[] expectedNumResources(){
        ...
        for(int i=2; i<=12; i++) {
            SOCResourceSet rolled = new SOCResourceSet();
        	rolled.add(game.getResourcesGainedFromRoll(this, i));
        	for (int rtype = SOCResourceConstants.CLAY; rtype <= thisTurnExpRes.length; ++rtype) {
            	thisTurnExpRes[rtype-1] += rolled.getAmount(rtype)*prob(i);
            }
        }
        return thisTurnExpRes;
    }
    ...
    // Returns comma-delimited string of parameters from this game
    public String getStats() {
    	/*
    	String ret = this.name.contains("our")+","+this.buildingVP+","+this.devCards.getNumVPCards()+","+this.hasLargestArmy()+","+this.hasLongestRoad()+","+this.longestRoadLength+","+this.numKnights+","+this.playerNumber+","+this.settlements.size()+","+this.cities.size()+
    					","+this.devCards.getTotal()+","+this.roads.size()+","+this.resourceStats[SOCResourceConstants.CLAY]+","+this.resourceStats[SOCResourceConstants.WOOD]+","+this.resourceStats[SOCResourceConstants.SHEEP]+
    					","+this.resourceStats[SOCResourceConstants.ORE]+","+this.resourceStats[SOCResourceConstants.WHEAT]+","+this.ourNumbers.getNumbersForResource(SOCResourceConstants.CLAY)+","+this.ourNumbers.getNumbersForResource(SOCResourceConstants.WOOD)+
    					","+this.ourNumbers.getNumbersForResource(SOCResourceConstants.SHEEP)+","+this.ourNumbers.getNumbersForResource(SOCResourceConstants.ORE)+","+this.ourNumbers.getNumbersForResource(SOCResourceConstants.WHEAT);
    	
    	*/
    	int[] portArray = new int[ports.length];
    	for(int i=SOCBoard.MISC_PORT; i<ports.length; i++)
    		if(ports[i])
    			portArray[i] = 1;
    		else
    			portArray[i] = 0;
    	
    	double[] resourceDist = new double[SOCResourceConstants.WOOD];
    	for(int i=0; i<resourceDist.length; i++) {
    		resourceDist[i] = resourceStats[i+1] / totalResources;
    	}
    	
    	int hasLargestArmy = 0;
    	//int largestArmyDiff = 0;
    	if(this.hasLargestArmy())
    		hasLargestArmy = 1;
    	//else
    	//	largestArmyDiff = game.getPlayerWithLargestArmy().getNumKnights() - this.getNumKnights() + 1;
    	
    	game.getResourcesGainedFromRoll(this, 2);
    	
    	int hasLongestRoad = 0;
    	//int longestRoadDiff = 0;
    	if(this.hasLongestRoad())
    		hasLongestRoad = 1;
    	//else
    	//	longestRoadDiff = game.getPlayerWithLongestRoad().getLongestRoadLength() - this.longestRoadLength + 1;

    	
    	String ret = this.getTotalVP()+","+this.turnCount+","+this.settlements.size()+","+this.cities.size()+","+this.roads.size()+","+this.resourceStats[SOCResourceConstants.CLAY]+","+this.resourceStats[SOCResourceConstants.ORE]+","+this.resourceStats[SOCResourceConstants.SHEEP]+","+this.resourceStats[SOCResourceConstants.WHEAT]+","+this.resourceStats[SOCResourceConstants.WOOD]+","
    				;
    	...
    	for(int i=0; i<expectedResources.length; i++)
    		ret += expectedResources[i]+",";
    	
    	for(int i=0; i<expectedResources.length; i++)
    		ret += (expectedResources[i]*turnCount)+",";
    	
    	for(int i=SOCBoard.MISC_PORT; i<portArray.length; i++)
    		ret += portArray[i]+",";
    				
    	ret += hasLargestArmy+","+numKnights+","
    				//+largestArmyDiff+","
    				+hasLongestRoad+","+longestRoadLength;//+","+longestRoadDiff;
    	return ret;
    }
}
