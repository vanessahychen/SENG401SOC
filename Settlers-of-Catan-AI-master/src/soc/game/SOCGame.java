...
import soc.ourRobot.SOCOurBrain;
...
public class SOCGame implements Serializable, Cloneable
{
    ...
    /**
     * Added by Ryan: copy constuctor for SOCGame
     */
    public SOCGame(SOCGame g) {
    	inUse = g.inUse;
    	name = g.name;
    	active = g.active;
    	isLocal = g.isLocal;
    	hasOldClients = g.hasOldClients;
    	clientVersionLowest = g.clientVersionLowest;
    	clientVersionHighest = g.clientVersionHighest;
    	clientVersionMinRequired = g.clientVersionMinRequired;
    	isFromBoardReset = g.isFromBoardReset;
    	boardResetOngoingInfo = g.boardResetOngoingInfo; // do we ever need to reset?
    	boardResetVoteRequester = g.boardResetVoteRequester;
    	boardResetVotes = copy(g.boardResetVotes);
    	boardResetVotesWaiting = g.boardResetVotesWaiting;
    	board = new SOCBoard(g.board);
    	opts = copy(g.opts);
    	players = copy(g.players);
    	seats = copy(g.seats);
    	seatLocks = copy(g.seatLocks);
    	currentPlayerNumber = g.currentPlayerNumber;
    	firstPlayerNumber = g.firstPlayerNumber;
    	lastPlayerNumber = g.lastPlayerNumber;
    	maxPlayers = g.maxPlayers;
    	currentDice = g.currentDice;
    	gameState = g.gameState;
    	oldGameState = g.oldGameState;
    	placingRobberForKnightCard = g.placingRobberForKnightCard;
    	forcingEndTurn = g.forcingEndTurn;
    	askedSpecialBuildPhase = g.askedSpecialBuildPhase;
    	specialBuildPhase_afterPlayerNumber = g.specialBuildPhase_afterPlayerNumber;
    	playerWithLargestArmy = g.playerWithLargestArmy;
    	oldPlayerWithLargestArmy = g.oldPlayerWithLargestArmy;
    	playerWithLongestRoad = g.playerWithLongestRoad;
    	oldPlayerWithLongestRoad = (Stack) g.oldPlayerWithLongestRoad.clone();
    	playerWithWin = g.playerWithWin;
    	numDevCards = g.numDevCards;
    	devCardDeck = copy(g.devCardDeck);
    	rand = g.rand;
    	allOriginalPlayers = g.allOriginalPlayers;
    	startTime = (Date) g.startTime.clone();
    	expiration = g.expiration;
    	turnCount = g.turnCount;
    	roundCount = g.roundCount;
    }
    
    // Added by Ryan, returns a copy of a SOCPlayer array
    private SOCPlayer[] copy(SOCPlayer[] arr) {
    	if(arr == null)
    		return null;
    	SOCPlayer[] ret = new SOCPlayer[arr.length];
    	for(int i=0; i<ret.length; i++)
    		ret[i] = new SOCPlayer(arr[i], this);
    	return ret;
    }
    ...
    // added by Katie:
    // doesn't do all the checking and game state changes
    public void putPiece2(SOCPlayingPiece pp)
    {
        /**
         * call putPiece() on every player so that each
         * player's updatePotentials() function gets called
     ...
     * @return the card that was drawn
     */
    public void buyDevCard2(int pn)
    {
        numDevCards--;
        ...
        resources.subtract(1, SOCResourceConstants.SHEEP);
        resources.subtract(1, SOCResourceConstants.WHEAT);
        players[pn].getDevCards().add(1, SOCDevCardSet.NEW, SOCDevCardConstants.KNIGHT);
        //checkForWinner();
    ...
    /** Added by Katie. Returns number of player with >= 10 VP, -1 if none 
     * Concern: we don't know any of the other players dev cards so we can't
     * really get the total VP */
    
    public int winningPlayer(){
    	for(int i=0; i<maxPlayers; i++){
    		if(players[i].getTotalVP() >= VP_WINNER){
    			return i;
    		}
    	}
    	return -1;
