...
public class SOCDevCardSet implements Serializable, Cloneable
{
    ...
    /** Added by Katie
     * @return total number of each type of card
     */
    public int[] getTotalTypes(){
    	int[] cards= new int[SOCDevCardConstants.MAX_KNOWN+1];
    	for (int i = SOCDevCardConstants.MIN; i<= SOCDevCardConstants.MAX_KNOWN; i++){
    		cards[i]= devCards[OLD][i]+devCards[NEW][i];
    	}
    	return cards;
    }
