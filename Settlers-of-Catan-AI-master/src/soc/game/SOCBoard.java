...
public class SOCBoard implements Serializable, Cloneable
{
    ...
    /**
     * Added by Ryan: constructor to copy SOCBoard
     * @param board the SOCBoard we want to copy
     */
    public SOCBoard(SOCBoard b) {
		boardWidth = b.boardWidth;
		boardHeight = b.boardHeight;
		minNode = b.minNode;
		minEdge = b.minEdge;
		maxEdge = b.maxEdge;
		boardEncodingFormat = b.boardEncodingFormat;
		hexLayout = copy(b.hexLayout);
		portsLayout = copy(b.portsLayout);
		numberLayout = copy(b.numberLayout);
		numToHexID = copy(b.numToHexID);
		hexIDtoNum = copy(b.hexIDtoNum);
		nodeIDtoPortType = copy(b.nodeIDtoPortType);
		robberHex = b.robberHex;
		ports = copy(b.ports);
		pieces = (Vector) b.pieces.clone();
		roads = (Vector) b.roads.clone();
		settlements = (Vector) b.settlements.clone();
		cities = (Vector) b.cities.clone();
		rand = b.rand;
		nodesOnBoard = copy(b.nodesOnBoard);
	}
    ...
    public SOCBoard copy() throws CloneNotSupportedException {
    	return (SOCBoard) super.clone();
    }
}
