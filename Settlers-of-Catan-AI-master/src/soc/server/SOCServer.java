...
public class SOCServer extends Server
{
    ...
    // Added by Katie
    /**
     * Robot paramters to run our robot. (For practice games; not referenced by server)
     * Same as ROBOT_PARAMS_DEFAULT but with OUR_STRATEGY, not FAST_STRATEGY. Will create
     * an instance of SOCOurBrain instead of SOCRobotBrain
     *
     * @see #ROBOT_PARAMS_DEFAULT
     * @see soc.robot.SOCRobotDM
     */
    public static SOCRobotParameters ROBOT_PARAMS_OURS
    	= new SOCRobotParameters(120, 35, 0.13f, 1.0f, 1.0f, 3.0f, 1.0f, 2, 1);
    ...
    // Changed by Katie to create one of our bots
    /**
    ...
    public void serverUp()
    {
    	...
    	//setupLocalRobots(1,1,1);  // each bot gets a thread
    }
...
/*
    ...
    //TODO: get rid of randomness here. Pick one as our robot, others as theirs
    ...
    {
    	//SOCRobotClient[] robo_ours = new SOCRobotClient[numOurs];
        ...
        for (int i = 0; i < numFast; ++i)
        {
            ...
            SOCPlayerLocalRobotRunner.createAndStartRobotClientThread(rname, strSocketName, port, true);
        ...
        {
            String rname = "our_robot " + (i+1+robo_fast.length);
            SOCPlayerLocalRobotRunner.createAndStartRobotClientThread(rname, strSocketName, port, false);
                // includes yield() and sleep(75 ms) this thread.
        }
        ...
        SOCServer.ROBOT_PARAMS_DEFAULT = SOCServer.ROBOT_PARAMS_OURS; 
        for (int i =0; i < numOurs; i++){
        	String rname= "killer " + (i+1+robo_fast.length + robo_smrt.length);
     ...
     * @param mes  the messsage
     */
    // TODO: Ryan rewrote this method below
    /*
    ...
    */
    ...
    // TODO: new start game, should only use robots
    // kick out seated players, have robots take seats
    private void handleSTARTGAME(StringConnection c, SOCStartGame mes)
    {
        if (c != null)
        {
            ...
            if (ga != null)
            {
                ...
                try
                {
                    if (ga.getGameState() == SOCGame.NEW)
                    {
                    	...
                    	// TODO: Ryan: boot all seated players
                    	
                    	for (int i = 0; i < ga.maxPlayers; i++)
                        {
                            if (!ga.isSeatVacant(i))
                            {
                                ga.removePlayer(ga.getPlayer(i).getName());
    ...
    private void endGameTurn(SOCGame ga, SOCPlayer pl)
    {
        ...
        // If a winner, write stats for each player
        if(ga.getGameState() == SOCGame.OVER) {
        	try {
	            ...
	            BufferedWriter bw = new BufferedWriter(out_stream);
				SOCGameStats gamestats = new SOCGameStats(gname, scores, isRobot);

				stats += gamestats.toString();
				
				
				for(int i=0; i<ga.MAXPLAYERS; i++) {
					 // player's stats
					SOCPlayer cp = ga.getPlayer(i);
					if (cp != null) {
						bw.write(cp.getStats());
						System.out.println("*********************************************");
						System.out.println("Player "+i+": "+cp.getStats());
						bw.newLine();
					}
					else {
						bw.write("Player "+i+" was null!");
						System.out.println("*********************************************");
						System.out.println("Player "+i+" was null!");
						bw.newLine();
					}
				...
				bw.write(stats);
				D.ebugPrintln("*********************************************");
				D.ebugPrintln("Game stats: "+stats);
				bw.newLine();
				D.ebugPrintln("Wrote: "+stats);
				bw.close();
     ...
     * @throws IOException 
     */
    protected void sendGameStateOVER(SOCGame ga) throws IOException
    {
    	D.ebugPrintln("GAME OVER");
        final String gname = ga.getName();
        String msg;
        ...
        for (int i = 0; i < ga.maxPlayers; i++)
        {
            ...
            D.ebugPrintln();
    		D.ebugPrintln();
    		D.ebugPrintln("GAME OVER: Player #"+i+" had "+pl.getPublicVP()+" VPs");
    		D.ebugPrintln();
    		D.ebugPrintln();
			...
			stats = "Individual stats: ";
				...
				if (pl != null) {
					bw.write(pl.getStats());
					System.out.println("*********************************************");
					System.out.println("Player "+i+": "+pl.getStats());
					bw.newLine();
				}
				else {
					bw.write("Player "+i+" was null!");
    ...
    private static class SOCPlayerLocalRobotRunner implements Runnable
    {
        ...
        // TODO: Ryan changed this method to run with our robot
        // 'add option' means that we should make a parameter for which RobotClient to use 
        
        // Ryan: change made in SOCRobotClient.handleJOINGAMEAUTH()
        ...
        public static void createAndStartRobotClientThread
            (final String rname, final String strSocketName, final int port, final boolean useTheirs)
        {
            SOCRobotClient rcli;
        	
            if (strSocketName != null)
            	rcli = new SOCRobotClient (strSocketName, rname, "pw", useTheirs);
            else {
            	rcli = new SOCRobotClient ("localhost", port, rname, "pw", useTheirs);
            }
