...
public class SOCRobotBrain extends Thread
{
    ...
    public void run()
    {
        ...
        if (pinger != null)
        {
            ...
            try
            {
                ...
                //boolean res= this instanceof SOCOurBrain;
                //client.sendText(game, )
                
                int strategy= robotParameters.getStrategyType();
                ...
                while (alive)
                {
                    ...
                    if ((game.getGameState() == SOCGame.WAITING_FOR_DISCOVERY) && (!waitingForGameState))
                    {
                        ...
                        if ((!waitingForOurTurn) && (ourTurn))
                        {
                            if (!(expectPLAY1) && (counter < 4000))
                            {
                                ...
                                expectPLAY1 = true;
                                counter = 0;
                                // TODO Possible game AI in run() [where is resourceChoices set?]
                    ...
                    if ((game.getGameState() == SOCGame.WAITING_FOR_MONOPOLY) && (!waitingForGameState))
                    {
                        ...
                        if ((!waitingForOurTurn) && (ourTurn))
                        {
                            if (!(expectPLAY1) && (counter < 4000))
                            {
                                ...
                                expectPLAY1 = true;
                                counter = 0;
                                // TODO Possible game AI in run() [where is monopolyChoise set?]
                 ...
                 // TODO Possible game AI in run() [structural AI]
     ...
     * @since 1.1.08
     */
    protected void buildRequestPlannedPiece(SOCPossiblePiece targetPiece)
    {
        buildingPlan.pop();
    ...
    public void pause(int msec)
    {
        ...
        // TODO: Ryan: got rid of sleep
        try
       {
            Thread.yield();
            
            
            if(!waitingForTradeResponse && !waitingForTradeMsg)
            	sleep(Math.min(msec, 100));
            else
    ...
    protected void moveRobber()
    {
         ...
         * to pick the 'least bad' one (consider that it would be late in the game).
